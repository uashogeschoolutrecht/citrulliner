...
line_graph_indiv_GRINTA <- function(DF, 
                                    palette_graph, 
                                    ymax, 
                                    ymin){
  ...
  nameFile <- paste(center, nameLine, study, sep = "_")
  ...
  long_name <- analyte_annotations %>%
   ...
   plot_title <- firstup(paste(long_name, study, center)) 
   #############################
   # axis limits
   #############################
...
#                         name="Cavalcanti")
  ...
  scale_colour_manual(labels = c("Rest", 
                                 "70%",
                                 "70% DH",
                                 "50%",
                                 "Interval"),
  ...
  theme_individual() +
   ...
   xlab("Time (hours)") + 
      ...
      ylab(paste("Concentration", paste0("(", 
                                       y_axis_label,
                                       ")"))) +
...
#  eps <- paste(nameFile,"line.eps", sep = "") 
 ...
 saveInImageDirectory(svg)
 # saveInImageDirectory(eps)
  saveInImageDirectory(png)
